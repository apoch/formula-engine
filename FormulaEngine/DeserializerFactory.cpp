...
//
// Provides complete support for loading FormulaEngine scripts
// from files on disk. Additionally, exposes some functions to
// load subsets of a script.
//
// This specific implementation uses JSON to represent scripts
// in serialized form. There is no particularly good reason to
// favor JSON; it's simply widely recognized and easy to parse
// with off-the-shelf libraries. Actual deployments of formula
// based engines should seriously prefer using deserialization
// facilities from the larger engine ecosystem instead.
//
// Note that this code is fragile and lame in that it silently
// discards malformed inputs. It would be a fantastic project,
// hint hint, for someone to come through and make this module
// fault-tolerant and add some diagnostics in case of bogosity
// in the input data.
//
...
//
// Deserialize a set of script-to-engine bindings
//
// Populates the given scriptable with the binding tokens obtained
// by registering the binding names with the world's token manager
//
...
//
// Deserialize a set of canned text strings
//
...
//
// Deserialize a set of script actions
//
// This is one of the more painfully hard-coded sections of deserialization.
// Ideally a larger engine would provide some reflection-style capabilities,
// allowing actions to be loaded without a "central point of failure," as it
// is here currently.
//
// Part of the motivation for doing it this way is to avoid leaking a ton of
// deserialization logic into every single script action implementation. The
// tradeoff is that every script action has to be statically knowable by the
// deserialization module. Perhaps more substantial applications of formula-
// based simulation would make a different tradeoff.
//
static void LoadArrayOfActions(const char name[], const picojson::object & obj, ActionSet * actions, ScriptWorld * world, FormulaParser * parser) {
	...
	for (auto & actionentry : actioniter->second.get<picojson::array>()) {
		...
		if (actionkey == "AddToList") {
			...
			// There is no action matching the specified action key string
			assert(false);
		}
	}
}
...
//
// Deserialize a set of formula-specified properties onto a scriptable object
//
...
//
// Deserialize a set of pre-canned object lists
//
...
//
// Deserialize a set of pre-canned scriptable objects
//
...
//
// Deserialize a script file into the given world
//
// It is permissible to load multiple files into the same world.
//
...
//
// Deserialize a set of event handlers
//
